...
class TestQueryBuilder < Minitest::Test
  ...
  def test_initialize
    ...
    assert_equal [], @builder.operation_chain
    assert_equal({}, @builder.variables)
  end
  ...
  def test_chain_operation_multiple
    result = @builder
      ...
      .chain_operation("withExec", { "args" => [ "echo", "hello" ] })
    query = result.build_query_with_selection("stdout")
    assert_equal "query { container { from(image: \"alpine:latest\") { withExec(args: [\"echo\", \"hello\"]) { stdout } } } }", query.gsub(/\s+/, " ").strip
  end
  ...
  def test_complex_chained_query
    result = @builder
      .chain_operation("from", { "image" => "alpine:latest" })
      .chain_operation("withExec", { "args" => [ "apk", "add", "curl" ] })
      .chain_operation("withExec", { "args" => [ "curl", "example.com" ] })
    query = result.build_query_with_selection("stdout")
    assert_equal "query { container { from(image: \"alpine:latest\") { withExec(args: [\"apk\", \"add\", \"curl\"]) { withExec(args: [\"curl\", \"example.com\"]) { stdout } } } } }", query.gsub(/\s+/, " ").strip
  end
end
