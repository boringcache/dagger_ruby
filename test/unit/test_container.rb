...
class TestContainer < Minitest::Test
  ...
  def test_with_directory_mounts_directory
    ...
    query = new_container.instance_variable_get(:@query_builder)
    assert_equal [ { field: "withDirectory", args: { "path" => path, "directory" => "dir_123" } } ], query.operation_chain
  end
  ...
  def test_with_secret_env_sets_secret_environment
    ...
    query = new_container.instance_variable_get(:@query_builder)
    assert_equal [ { field: "withSecretEnv", args: { "name" => name, "secret" => "secret_123" } } ], query.operation_chain
  end
  ...
  def test_with_exposed_port_exposes_port
    ...
    query = new_container.instance_variable_get(:@query_builder)
    assert_equal [ { field: "withExposedPort", args: { "port" => port, "protocol" => protocol } } ], query.operation_chain
  end
  ...
  def test_query_builder_integration
    ...
    assert_equal [], query_builder.operation_chain
  end
end
